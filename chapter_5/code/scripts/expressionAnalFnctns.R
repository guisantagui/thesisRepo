...
# Does univariate analysis, either t test or mann whitney U test, between THP1 and BoMac cells
doUniVar <- function(data, statFun, plateNorm = T){
        hosts <- unique(data$host)
        if(plateNorm){
                pVals <- sapply(unique(data$lin),
                                function(x) statFun(data$relExp_plateNorm[data$lin == x & data$host == hosts[1]],
                                                    data$relExp_plateNorm[data$lin == x & data$host == hosts[2]])$p.value)
        }else{
                pVals <- sapply(unique(data$lin),
                                function(x) statFun(data$relExp[data$lin == x & data$host == hosts[1]],
                                                    data$relExp[data$lin == x & data$host == hosts[2]])$p.value)
        }
...
# Do boxplots.
doExpBoxPlot <- function(data, plateNorm = T, gene, stats = "none", formatTitle = T){
        geneNameDF <- data.frame(gene = c("plcA", "plcB", "plcC", "plcD"),
                                 id = c("Rv2351c", "Rv2350c", "Rv2349c", "Mb1784c"))
        data <- data[data$primerPair == gene, ]
        data$host <- gsub("BOMAC", "BoMac", data$host)
        data$host <- gsub("THP-1", "THP1", data$host)
        data$lin <- gsub("Bovis", "*M. bovis*", data$lin)
        data$lin <- gsub("Chimp", "Chimp. B.", data$lin)
        data$lin <- factor(data$lin, levels = c("*M. bovis*", "Chimp. B.", "L6", "L5"))
        data$host <- factor(data$host, levels = c("THP1", "BoMac"))
        if(plateNorm == FALSE){
                boxPlot <- ggplot(data, aes(x = lin, fill = lin, y = relExp))
        }else{
                boxPlot <- ggplot(data, aes(x = lin, fill = lin, y = relExp_plateNorm))
        }
        if(stats == "mann_whitney"){
                pVals <- doUniVar(data, wilcox.test, plateNorm = plateNorm)
        }else if(stats == "t_test"){
                pVals <- doUniVar(data, t.test, plateNorm = plateNorm)
        }
        if(formatTitle){
                plotTitle <- sprintf("%s (%s)",
                                     geneNameDF$gene[match(gene, geneNameDF$id)],
                                     gene)
        }else{
                plotTitle <- gene
        }
        ...
        if(stats != "none"){
                names(pVals) <- unique(data$lin)
                pVals <- pVals[match(levels(data$lin), names(pVals))]
                ...
                if(plateNorm){
                        maxYs <- sapply(levels(data$lin), function(x) max(data$relExp_plateNorm[data$lin == x]))
                }else{
                        maxYs <- sapply(levels(data$lin), function(x) max(data$relExp[data$lin == x]))
                }
                maxYs <- maxYs + max(maxYs) * 0.025
                signifDF <- data.frame(x = 1:(length(levels(data$lin))) - 0.2,
                                       xend = 1:(length(levels(data$lin))) + 0.2,
                                       y = maxYs,
                                       annotation = symbols,
...
pcBiplot <- function(PC, x="PC1", y="PC2", varPlotFilt = NULL, formatGeneNames = T, labels = F){
        ...
        data$Host <- gsub("thp1", "THP1", data$Host)
        data$Host <- factor(data$Host, levels = c("THP1", "BoMac"))
...
# Do regression plot of univariate model
doRegPlot <- function(model, varY, varX){
        fStat <- summary(model)$f
        ...
        bactCols <- data.frame(Bacteria = c("L5", "L6", "*M. bovis*", "Chimp. B."),
                               color = c("#871414", "#24ad37", "#f279ce", "#c4bf62"),
                               stringsAsFactors = F)
        geneNames <- data.frame(locus = c("Rv2349c",
                                          "Rv2350c",
                                ...
                                gene = c("plcC",
                                         ...
                                         "plcA",
                                         "plcD"))
        citoFormat <- data.frame(var = c("early_apoptotic",
                                         "late_apoptotic",
                                         "necrotic",
                                         "viable",
                                         "virulenceNecrotViabl"),
                                 name = c("Early apoptosis fold change",
                                          "Late apoptosis fold change",
                                          "Necrosis fold change",
                                          "Viability fold change",
                                          "Virulence"))
        
        shapeVec <- c(16, 17)
        names(shapeVec) <- c("THP1", "BoMac")
        modelDF <- model$model
        ...
        modelDF$Bacteria <- gsub("Bovis", "*M. bovis*", modelDF$Bacteria)
        modelDF$Bacteria <- gsub("Chimp", "Chimp. B.", modelDF$Bacteria)
        modelDF$Bacteria <- factor(modelDF$Bacteria, 
                                   levels = unique(modelDF$Bacteria))
        modelDF$Host <- gsub("BOMAC|Bomac", "BoMac", modelDF$Host)
        ...
        plotShapes <- shapeVec[match(levels(modelDF$Host),
                                     names(shapeVec))]
        xLab <- sprintf("%s (%s) expression",
                        geneNames$gene[geneNames$locus == varX],
                        varX)
        yLab <- citoFormat$name[citoFormat$var == varY]
        ...
        if(model$coefficients[2] > 0.3){
                xPos <- min(xRange) + abs(min(xRange) * 0.8)
                yPos <- max(yRange) - abs(max(yRange) * 0.05)
        }else if(model$coefficients[2] <= 0.3 & model$coefficients[2] > 0){
                xPos <- max(xRange) - abs(max(xRange) * 0.6)
        ...
        plot <- plot +
                geom_label(x = xPos, y = yPos, label = eq, parse = TRUE)
        plot
}
