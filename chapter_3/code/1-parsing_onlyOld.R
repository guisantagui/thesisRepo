...
consNames <- as.character(dictionary$Consensus[match(colnames(metFormat)[2:ncol(metFormat)], 
                                                     dictionary$Old.Data.Names)])
consNames <- c(colnames(metFormat)[1], consNames)

!is.na(consNames)
# Remove the columns which new name is NA, as in the dictionary are the compounds that are redundant 
# (have same values)

colnames(metFormat)[!is.na(consNames)] <- consNames[!is.na(consNames)]
